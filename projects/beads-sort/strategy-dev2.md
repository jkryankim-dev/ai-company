...
정정 2026-08-19 (지시 002): 옵트인 33명·14일 시계 가동 확정에 따라 결론 1·A절 마감 계산을 다시 썼다.

## 결론 (3줄)
1. **최대 리스크는 targetSdk 마감 교차다.** 구글 실측: 2026-08-31부터 신규 제출은 API 36 필수.
   시계 가동 중(옵트인 33명) → 신청 가능일 ≈ 9/1, 마감과 **하루 차(±1일) 교차**. API 36 빌드 준비를 지금 시작해야 한다.
...
### 마감 계산 — 2026-08-19 지시 002 정정
- 전제 정정: 옵트인 **33명 완료·14일 시계 가동 중**(대표 확인). 종전 "미시작" 전제는 폐기.
- 입력: 신청 가능일 = **2026-09-01 전후**(context.md 7행. 시작일이 콘솔 값이라 ±1일) · 마감 = **2026-08-31**(API 36).
- 판정: **여전히 신청일 > 마감.** 다만 옛 계산("미시작 → 9/2 이후")과 달리 무기한 밀림 변수는 소멸하고
  교차 폭은 사실상 하루(±1일)로 고정. 남은 변수는 시계 시작일과 승격 허용 여부뿐이다.
- 경계: 14일이 8/30에 완료돼 8/31 당일 신청이 가능해도, 실측 문구 "Starting August 31"상
  그날부터 API 36 요건이 적용되는 것으로 읽힌다 — 당일 제출 면제 여부는 대표 확인.
- 대응(방향 유지, 해석 변경): 신청일 > 마감이 사실상 확정이므로 **API 36 빌드는 보험 아니라 본선 경로**다.
  ...
  - 분기점(대표 확인 1순위): 기존 1.0.2+3 AAB의 무업로드 프로덕션 승격이 마감 후에도 허용되는지.
    허용 → 9/1 전후 기존 AAB로 신청. 불허 → API 36 빌드 완성이 신청의 선행 조건.
  - 주의: 플레이북 거절 사유 6 "14일 딱 채우고 즉시 신청" — 9/1은 최소 대기일이지 신청 목표일이 아니다.
...
2. 확인: 마감 후 기존 AAB 프로모션 가능 여부 — 허용 여부가 9/1 전후 신청 경로(기존 AAB vs API 36 빌드)를 결정
